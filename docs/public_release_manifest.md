...
This is the sanitized public release assembled on 2026-07-16 and published at the GitHub repository. The repository is public and release `v0.1.1` contains the same reviewed snapshot after the dashboard path fix. The release is archived in Zenodo at [https://doi.org/10.5281/zenodo.21404580](https://doi.org/10.5281/zenodo.21404580).
...
The figures and aggregate tables are a working-analysis snapshot and should be checked against the final revised manuscript before citing a specific figure or table version.
...
The dashboard smoke test is designed to run from the repository root and reads only the aggregate files under `release_results/`. Before a future release, rerun a repository-wide secret/path scan, review the final author list and license, verify that credential-bearing source files have not been staged, and confirm that the Zenodo metadata and checksums remain aligned with the tagged release.
